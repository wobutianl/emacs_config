PEP: 8
Title: Style Guide for Python Code:Python 代码风格指导（本书本身就是一个范例）
Version: $Revision$
Last-Modified: $Date$
...
这个文档给出了Python发布的标准库的编码规范。
如果你要在Python中嵌入C语言，请看PEP7 的规范。
This document and PEP 257 (Docstring Conventions) were adapted from
Guido's original Python Style Guide essay, with some additions from
Barry's style guide [2]_.
此文档和PEP 257（Docstring规范）采用Guido原始的Python风格论文，和Barry's的风格指南。

This style guide evolves over time as additional conventions are
identified and past conventions are rendered obsolete by changes in
the language itself.
因新的规范加入和旧的过时的规范被改变，这个文档随着时间的前进而在改进中。
...
许多项目有他们自己的编码指南。这种情况下，项目的规范有优先权。
...
Guido的一个主要观点：代码--看的次数比写的次数要多得多。
此文档就是为了提高代码的可阅读性，保持不同环境下Python代码的一致性。
就如同：PEP 20所说的：代码可读性很重要

A style guide is about consistency.  Consistency with this style guide
is important.  Consistency within a project is more important.
Consistency within one module or function is most important.
这个指南就为了代码一致性。一致的用这个风格很重要。在项目中一致更重要。
在一个模块或者一个函数中的代码一致性最重要。
...
最重要的：知道什么时候不一致。有时候不需要应用这个指南。当有疑惑时，保持自己的判断。
看别的例子，决定哪个看起来最好。别犹豫，别问！

In particular: do not break backwards compatibility just to comply with
this PEP!
特别提醒：别为了应用此文档，打破过去的兼容性
...
连续行指令，需要用Python的轮廓线对齐，结合小括号，中括号，大括号或者'hanging indent'
当利用悬停缩进时，请遵循以下原则：
第一行没有参数。
进一步缩，让它与其他行区分开来
Yes::
