...
#+TAGS: { LEARN(l) ENTERTAIN(e) JOB(j) OTHER(o) }
#+TAGS: { READING(r) ENGLISH(E) CODE(c) PAPER(p) }
#+TAGS: { FILM(f) MUSIC(m) WEB(w) SPORT(s) GAME(g) QQ(q) }
#+TAGS: { SHOPPING(S) LIFE(L) DAIRY(d) }
#+TAGS: { 每周工作四小时  公正 }
